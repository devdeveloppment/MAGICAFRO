...
def initiate_cinetpay_payment(request, order_id):
    ...
    transaction_id = f"MA-{order.id}-{int(time.time())}"
    ...
    data = {
        ...
        "site_id": str(settings.CINETPAY_SITE_ID),
        "transaction_id": transaction_id,
        "amount": amount_xof,
        "currency": "XOF", 
        "description": f"MagicAfro #{order.id}",
        ...
        "metadata": f"{order.id}",
        "lang": "fr",
        "customer_id": str(order.user.id) if order.user else f"GUEST-{order.id}",
        ...
        "customer_address": order.street_address or "Lomé",
        "customer_city": order.city or "Lomé",
        "customer_country": "TG", 
        "customer_state": order.city or "Lomé",
        "customer_zip_code": order.postal_code or "0000",
    }
